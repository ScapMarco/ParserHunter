...
def get_ACFG(cfg, project, path_python_executable, path_script_asm2vec):
...
def get_Geometric_Data_from_CFG(cfg, project, label=None, path_python_executable=None, path_script_asm2vec=None, ref=None):
...
def get_Geometric_Datas(project, functions_addresses, path_python_executable, path_script_asm2vec, dictionary_labeled=True):
...
def print_CFG(cfg, title, name, hex_address):    
    print(f"\nPRINT CFG")
...
def print_CFG2(cfg):
    print(f"\n PRINT ACFG")
    # Define layout for visualizing the CFG
    layout = nx.spring_layout(cfg)
    # Create a Matplotlib figure and axis
    fig, ax = plt.subplots(figsize=(12, 8))
    # Draw the CFG using Matplotlib
    nx.draw(cfg, pos=layout, with_labels=True, node_size=2000, node_color='lightblue', font_size=10, ax=ax)
    ax.set_title("Attributed Control Flow Graph")
    plt.show()