...
def get_predicted_functions(project, functions_addresses, filename, llm_client):
    """
    Analyze each function in an Angr project, decompile it, query the LLM,
    and return prediction dicts including real labels.
    """
    ...
    for name, (address, true_label) in functions_addresses.items():
        ...
        # Try decompilation
        try:
            decompiled = project.analyses.Decompiler(func)
            pseudo_c_code = decompiled.codegen.text if decompiled.codegen else None
        except Exception as e:
            print(f"Decompilation failed for {name}: {e}")
            pseudo_c_code = None


        print(f"The decompiled code of function {name} at {hex(address)}:\n{pseudo_c_code}\n")
        ...
        if pseudo_c_code:
            prompt = (
                "You are a binary analysis assistant. I will give you the decompiled C code of a function.\n"
...
def initialize_output_csv(path_csv):
    """Create the CSV with header only if not existing."""
...
def append_predictions_to_csv(predictions, path_csv):
    """Append prediction rows to the global CSV."""
    with open(path_csv, "a", newline="", encoding="utf-8") as f:
        ...
        for entry in predictions:
            writer.writerow([
                ...
                entry["name"],
                entry["address"],
                entry["decompiled_code"] if entry["decompiled_code"] else "",
...
def main(path_bash_scripts, llm_client):
    OUTPUT_CSV = "../Results/llm_classification_qwen3_coder_30b/global_predictions_decompiled_code.csv"
