...
def get_assembly_code(func) -> str | None:
    """
    Extract a clean, continuous assembly code string for the entire function.
    Ignores block addresses and headers, only keeps the assembly instructions.
    """
    ...
    try:
        assembly_lines = []
        # Extract the Assembly code of the function
        for block in func.blocks: # in func.blocks we have the basic blocks
            # Get the VEX IR of the basic block
            assembly = block.capstone # block.capstone.pp()   # pretty-printed assembly
            assembly_lines.append(str(assembly))

        if not assembly_lines:
            return None
        return "\n".join(assembly_lines)
    except Exception as e:
        print(f"[Assembly] Extraction FAILED for function at 0x{func.addr:x}: {e}")
        return None
...
def get_predicted_functions(project, functions_addresses, filename, llm_client):
    ...
    for name, (address, true_label) in functions_addresses.items():
        ...
        # ----------------------------------------------------
        assembly_code = get_assembly_code(func)

        print(f"The assembly code of function {name} at {hex(address)}:\n{assembly_code}\n")
        
        # ----------------------------------------------------
        # LLM Classification on Assembly Code
        # ----------------------------------------------------
        if assembly_code:
            prompt = (
                "You are a binary analysis assistant. I will give you the assembly code of a function.\n"
        ...
        predicted_functions.append({
            ...
            "function_name": name,
            "address": hex(address),
            "assembly_code": assembly_code if assembly_code else "",
...
def main(path_bash_scripts, llm_client):
    OUTPUT_CSV = "../Results/llm_classification_qwen3_coder_30b/global_predictions_assembly_code.csv"
